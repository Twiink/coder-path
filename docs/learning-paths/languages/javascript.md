...
这是一条独立的 JavaScript 学习地图：从语言地基一路走到浏览器、工程化、安全和运行时原理。每一站都把常用能力、容易漏掉的边界和值得深挖的机制摆在桌面上；按顺序走，你会得到一条能自己闭环的路线，而不是一张只会指路的目录。
...
- **[TypeScript](/learning-paths/languages/typescript)** ---- 给 JS 加上类型系统，大厂标配，类型安全和开发体验的双重提升
...
- **[Vite](/learning-paths/build-tools/vite)** / **[Webpack](/learning-paths/build-tools/webpack)** ---- 现代前端项目的构建工具，从开发到生产的完整链路

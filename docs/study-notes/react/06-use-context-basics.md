...
2、在父组件中添加共享数据对象的引用，通过且只能通过<XxxContext.provider value=&#123;对象(xx属性)&#125;></XxxContext.provider>的形式将数据传递给子组件。请注意传值必须使用value=&#123;obj&#125;这种形式，若值本身为字符串则可以改为 value='xxx'；  

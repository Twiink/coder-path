...
parent: "[[目录]]"
children:
  - "[[前端/React/01 React Hook 简介]]"
  - "[[前端/React/02 useState基础用法]]"
  - "[[前端/React/03 useState高级用法]]"
  - "[[前端/React/04 useEffect基础用法]]"
  - "[[前端/React/05 useEffect高级用法]]"
  - "[[前端/React/06 useContext基础用法]]"
  - "[[前端/React/07 useContext高级用法]]"
  - "[[前端/React/08 useReducer基础用法]]"
  - "[[前端/React/09 useReducer高级用法]]"
  - "[[前端/React/10 useCallback基础用法]]"
  - "[[前端/React/11 useMemo基础用法]]"
  - "[[前端/React/12 useRef基础用法]]"
  - "[[前端/React/13 useImperativeHandle基础用法]]"
  - "[[前端/React/14 useLayoutEffect基础用法]]"
  - "[[前端/React/15 useDebugValue基础用法]]"
  - "[[前端/React/16 自定义hook]]"
  - "[[前端/React/17 React Hook 总结]]"
  - "[[前端/React/18 示例：React使用Echarts所用到的hooks]]"
  - "[[前端/React/19 useTransition基础用法]]"
  - "[[前端/React/附01：React基础知识]]"
...
## 学习路径补充（融合自 `React框架学习.md`）

当前这篇更偏参考手册。为了吸收另一篇分阶段教程的价值，这里补一个“怎么用这份笔记学习”的路径。

### 推荐顺序

1. 先看 React 的特点、组件通信、数据流和 JSX 等基础概念。
2. 再进入内置 Hooks，把 `useState / useEffect / useContext / useReducer` 打牢。
3. 然后学习性能与复用相关内容，例如 `useMemo / useCallback / 自定义 Hooks`。
4. 最后再看 Fiber、并发特性和复杂组件封装。

### 配套工程化视角

- 环境搭建优先使用 Vite。
- 组件开发时优先函数组件。
- 把“Props vs State”“父子通信”“列表与条件渲染”当成基础训练项。
- 进阶时再看路由拆分、状态管理、异步数据流和性能排查。

### 这两类文档的融合定位

- 本文继续作为“可回查的 React 总手册”。
- 原 `React框架学习.md` 中更强的“课程顺序感”，已经浓缩进本节，不再单独保留一篇总览型笔记。
